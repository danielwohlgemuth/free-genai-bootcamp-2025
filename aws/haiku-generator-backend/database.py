...
def retrieve_haikus(user_id: str)-> List[Haiku]:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM haiku WHERE user_id = ?', (user_id,))
...
def retrieve_haiku(user_id: str, haiku_id: str) -> Haiku:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM haiku WHERE user_id = ? AND haiku_id = ?', (user_id, haiku_id))
...
def retrieve_haiku_line(user_id: str, haiku_id: str, line_number: int) -> str:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(f'SELECT haiku_line_en_{line_number} FROM haiku WHERE user_id = ? AND haiku_id = ?', (user_id, haiku_id))
...
def retrieve_chats(user_id: str, haiku_id: str) -> List[Chat]:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM chat WHERE user_id = ? AND haiku_id = ? ORDER BY created_at ASC', (user_id, haiku_id))
...
def retrieve_last_chat(user_id: str, haiku_id: str) -> Chat | Empty:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM chat WHERE user_id = ? AND haiku_id = ? ORDER BY created_at DESC LIMIT 1', (user_id, haiku_id))
...
def update_haiku_lines(user_id: str, haiku_id: str, haiku: List[str], topic: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('UPDATE haiku SET haiku_line_en_1 = ?, haiku_line_en_2 = ?, haiku_line_en_3 = ?, topic = ? WHERE user_id = ? AND haiku_id = ?', (haiku[0], haiku[1], haiku[2], topic, user_id, haiku_id))
    conn.commit()
    conn.close()
...
def update_image_description(user_id: str, haiku_id: str, description: str, line_number: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(f'UPDATE haiku SET image_description_{line_number} = ? WHERE user_id = ? AND haiku_id = ?', (description, user_id, haiku_id))
    conn.commit()
    conn.close()
...
def update_translation(user_id: str, haiku_id: str, translated_haiku: str, line_number: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(f'UPDATE haiku SET haiku_line_ja_{line_number} = ? WHERE user_id = ? AND haiku_id = ?', (translated_haiku, user_id, haiku_id))
    conn.commit()
    conn.close()
...
def update_haiku_link(user_id: str, haiku_id: str, number: int, image_link: str = None, audio_link: str = None):
    ...
    if image_link is not None:
        cursor.execute(f'UPDATE haiku SET image_link_{number} = ? WHERE user_id = ? AND haiku_id = ?', (image_link, user_id, haiku_id))
    if audio_link is not None:
        cursor.execute(f'UPDATE haiku SET audio_link_{number} = ? WHERE user_id = ? AND haiku_id = ?', (audio_link, user_id, haiku_id))
    conn.commit()
    conn.close()
...
def set_status(user_id: str, haiku_id: str, status: str, error_message: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('UPDATE haiku SET status = ?, error_message = ? WHERE user_id = ? AND haiku_id = ?', (status, error_message, user_id, haiku_id))
    conn.commit()
    conn.close()
...
def delete_haiku_db(user_id: str, haiku_id: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM haiku WHERE user_id = ? AND haiku_id = ?', (user_id, haiku_id))
    cursor.execute('DELETE FROM chat WHERE user_id = ? AND haiku_id = ?', (user_id, haiku_id))
    conn.commit()
    conn.close()
